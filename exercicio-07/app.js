...
let drinkTypeMessege = null

switch (drinkType) {
  case 'água':
    drinkTypeMessege = 'Substância química cujas moléculas são formadas por dois átomos de hidrogênio e um de oxigênio.'
    break
  case 'refrigerante':
    drinkTypeMessege = 'Bebida não alcoólica e não fermentada, fabricada industrialmente, à base de água mineral e açúcar.'
    break
  case 'suco':
    drinkTypeMessege = 'Bebida produzida do líquido extraído de frutos.'
    break
  default:
    drinkTypeMessege = 'Bebida desconhecida.'
}

// console.log(drinkTypeMessege)
...
const number = 1
const numbersMessage = 'O valor de "number" é'

switch (number) {
  case 0:
    console.log(`${numbersMessage} ${number}`)
    break
  case 1:
    console.log(`${numbersMessage} ${number}`)
    break 
  default:
    console.log(`${numbersMessage} qualquer número, exceto 0 e 1`)
}
